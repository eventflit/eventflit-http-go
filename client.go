...
var eventflitPathRegex = regexp.MustCompile("^/apps/([0-9]+)$")

/*
Client to the HTTP API of Eventflit.

There easiest way to configure the library is by creating a new `Eventflit` instance:
...
To ensure requests occur over HTTPS, set the `Encrypted` property of a `eventflit.Client` to `true`.
...
Changing the `eventflit.Client`'s `Host` property will make sure requests are sent to your specified host.
	...
	client.Host = "foo.bar.com" // by default this is "service.eventflit.com".

If you wish to use push notifications, you need to define the Client.PushNotificationHost,
please see Eventflit docs for more details: https://eventflit.com/docs/push_notifications
*/
type Client struct {
	AppId                string
	Key                  string
...
/*
Clients can be instantiated from a specially-crafted Eventflit URL. For example:
...
func ClientFromURL(url string) (*Client, error) {
	...
	matches := eventflitPathRegex.FindStringSubmatch(url2.Path)
	if len(matches) == 0 {
...
This is particularly relevant if you are using Eventflit as a Heroku add-on,
which stores credentials in a `"EVENTFLIT_URL"` environment variable. For example:
...
`socket_id` from receiving the event. You can read more here: http://eventflit.com/docs/duplicates.
...
count is not allowed by default. Please contact us at http://support.eventflit.com
...
/*
Application security is very important so Eventflit provides a mechanism
...
For more information see our docs: http://eventflit.com/docs/authenticating_users.
...
In this library, one does this by passing a `eventflit.MemberData` instance.
...
/*
On your dashboard at http://app.eventflit.com, you can set up webhooks to POST
...
or after client-originated events. For more information see https://eventflit.com/docs/webhooks.

This library provides a mechanism for checking that these POST
requests are indeed from Eventflit, by checking the token and
authentication signature in the header of the request.

If the webhook is valid, a `*eventflit.Webhook* will be returned, and the `err` value will be nil.
...
func (c *Client) Webhook(header http.Header, body []byte) (*Webhook, error) {
	...
	for _, token := range header["X-Eventflit-Key"] {
		if token == c.Key && checkSignature(header.Get("X-Eventflit-Signature"), c.Secret, body) {
...
you have provided a Client.PushNotificationHost, please see Eventflit docs for details: https://eventflit.com/docs/push_notifications
*/
func (c *Client) Notify(interests []string, pushNotification PushNotification) (*NotifyResponse, error) {
	...
	path := fmt.Sprintf("/%s/%s/publishes", PushNotifAPIPrefixDefault, c.AppId)
